...
- **The band model is EXACT only for narrow bands, and the load class splits in two.** Measured over 6,288 SFA retail
  functions and re-checked against the GC/2.0 corpus: within the load class, values first written by a **computation**
  fill `r31`/`f31` **downward** (GPR 9.9:1, FP 3.7:1), but values first written by a **load or an immediate** fill the
  band **upward from the bottom** (FP 1.5:1 up; corpus 3.3:1 up) — the same direction as the copy class, because both
  are *materialized* rather than computed. Practical consequence: **an FP pool constant moves to a HIGHER `f` register
  by being materialized LATER**, the inverse of what "assigned `r31` downward" implies. The GPR-materialized quadrant
  is noise (SFA 1.6:1 down vs corpus 1.4:1 up) — build no lever on it. And the whole model degrades with band width:
  jointly (materialized ascending + computed descending) it explains **99.8% of GPR functions at 2 saved registers,
  98.1% at 3, 86.0% at 4, 48.1% at 5, 27.0% at 6+** (FP 86.5/63.8/55.9/37.5/25.6). At >=6 there is no total order to
  steer — four exhaustive declaration sweeps (720/225/144/121 candidates) returned zero movement, every one a wide-band
  function. **Screen before sweeping: `python3 tools/bandscreen.py --struct-only`** ranks the frontier by band width and
  by whether the mnemonic stream (not just the operands) differs; see `docs/band_width_worklist.md`.
