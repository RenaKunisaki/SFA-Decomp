...
#define WM_COLUMN_STATE_SIZE 0xA
...
/* Runtime state is owned by the carryable interface. */
typedef struct WMColumnState {
    u8 carryableState[WM_COLUMN_STATE_SIZE];
} WMColumnState;
...
STATIC_ASSERT(offsetof(WMColumnState, carryableState) == 0x0);
STATIC_ASSERT(sizeof(WMColumnState) == WM_COLUMN_STATE_SIZE);
