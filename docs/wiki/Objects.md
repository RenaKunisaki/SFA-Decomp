...
`0x34` map (u8), `0x35` mtxIdx, `0x36` newOpacity, `0x37` opacity | `hostedMapSlot` (u8) @0x34, `transformMatrixIndex` (s8) @0x35, `alpha` (u8) @0x36, `renderAlpha` @0x37 | **wiki's "map" is resolved by the producer**: `shader.c`'s `mapLoadForObject` scans `gLoadedRomListPages` from index 0x50 for 40 slots, `stb`s the winning slot at +0x34 (retail `li r30,80` / `li r0,40` / `stb r0,52(r28)`), registers it with `setMapActLut(mapId, slot)` and `updateObjGroups(slot)`; `Obj_FreeObject` releases it via `mapUnloadRomListPage(*(u8*)(obj+0x34))`. So this is the romlist page slot the object *hosts*, in the extended 0x50..0x77 band that `SaveGame_getMapAct`/`SaveGame_gplayGetObjGroupStatus` translate back through `gExtendedMapActLookup[0x28]`. `mtxIdx`/`newOpacity` unchanged
...
`0xAC` `MapId8 mapId` | `s8 mapEventSlot` | offset matches (`mapEventSlot == 0xAC`); **relationship to 0x34 now confirmed**: 0xAC is written once, by `loadCharacter` (`tmpl.fac = arg2`), from the `mapLayer` argument of `Obj_SetupObject` - the map-event slot the object *belongs to*, never re-written afterwards. It is the same index space as 0x34: `shader.c` takes each ObjGroup-6 host's `hostedMapSlot` (`u32 mid2 = obj2->anim.hostedMapSlot`) and passes it as the `mapLayer` of every object it instantiates inside that host's page, so a child's 0xAC is its host's 0x34. Both therefore feed `getMapAct`/`setObjGroupStatus` legally; DLL literals compared against 0xAC (2, 8, 0xD, 0x15, 0x1B) are plain world-map ids
...
3. **`GameObject.paletteIndex` (offset 0xE8) is confirmed by its own setter to be a hint-text
   ...
   u8 hintTextIdx; /* obj+0xE8; renamed from paletteIndex - objSetHintTextIdx's own name and
                       ...
                       src/dlls/objects/290_CCTestInfot/CCTestInfot.c */
   ```
...
5. **A category-ID enum doesn't exist yet.** Every `anim.classId` comparison in this codebase
   ...
   #define OBJCATEGORY_TRICKY          0x0002 /* dlls/objects/251/251.c */
