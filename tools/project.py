...
def classify_source_category(name: str) -> str:
    return "sdk" if name.startswith(SDK_OBJECT_PREFIXES) else "game"
...
def generate_build_ninja(
    ...
    if build_config:
        ...
        missing_config_counts = {"game": 0, "sdk": 0}
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            if obj is None:
                if config.warn_missing_config and not build_obj["autogenerated"]:
                    category = classify_source_category(obj_name)
                    missing_config_counts[category] += 1
                    print(f"Missing {category} configuration for {obj_name}")
        ...
        missing_config_total = sum(missing_config_counts.values())
        if missing_config_total > 0:
            print(
                "Missing configuration summary: "
                f"{missing_config_counts['game']} game, "
                f"{missing_config_counts['sdk']} sdk"
            )
