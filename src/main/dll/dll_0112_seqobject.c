...
void SeqObject_update(GameObject* obj)
{
    ...
    if ((state->flags & SEQOBJECT_STATE_OPEN) == 0)
    {
        ...
        bitValue = mainGetBit(placement->triggerGameBit);
        ...
        if (bitValue != state->triggerBitState)
        {
            ...
            if (bitValue != 0)
            {
                if (placement->triggerId != -1)
                {
                    (*gObjectTriggerInterface)->setRunSequenceWorldSpace((int)obj, 0);
                    (*gObjectTriggerInterface)->runSequence(placement->triggerId, obj, -1);
                }
                if ((placement->flags & SEQOBJECT_FLAG_LATCH_SOURCE_CLEAR) == 0 &&
                    (placement->flags & (SEQOBJECT_FLAG_SET_SOURCE_ON_SEQUENCE | SEQOBJECT_FLAG_SET_SOURCE_ON_DONE)) == 0)
                {
                    mainSetBits(placement->openGameBit, 1);
                }
            }
    ...
    else if ((state->flags & SEQOBJECT_STATE_TRIGGER_SEQUENCE) != 0)
    {
        ...
        if ((placement->flags & SEQOBJECT_FLAG_USE_TRIGGER_PARAM) != 0)
        {
            (*gObjectTriggerInterface)->runSequence(placement->triggerId, obj, placement->sequenceParam);
        }
        else
        {
            (*gObjectTriggerInterface)->runSequence(placement->triggerId, obj, 1);
    ...
    else if ((placement->flags & SEQOBJECT_FLAG_LATCH_SOURCE_CLEAR) != 0 &&
