...
void SpiritDoorLock_update(GameObject* obj)
{
    ...
    *(SpiritDoorLockOrbitWords*)orbitOffset =
        *(const SpiritDoorLockOrbitWords*)gSpiritDoorLockOrbitOffsetBase;
    ...
    if (state->active == 0)
    {
        if (mainGetBit(placement->doneGameBit) == 0)
        {
            ...
            {
                f32 modelScale =
                    obj->anim.modelInstance->rootMotionScaleBase * (f32)(s32)placement->scale;
    ...
    else
    {
        ...
        {
            if (Vec_distance(&obj->anim.worldPosX, &orbitObjects[i]->anim.worldPosX) >
        ...
        {
            texture->offsetT =
                (s16)(texture->offsetT + gSpiritDoorLockTexScrollSpeed * framesThisStep);
            texture->offsetS =
                (s16)(texture->offsetS + gSpiritDoorLockTexScrollSpeed * framesThisStep);
            if ((s32)texture->offsetT > (s32)(gSpiritDoorLockTexScrollWrap << 8))
            {
                texture->offsetT =
                    (s16)(texture->offsetT - (gSpiritDoorLockTexScrollWrap << 8));
            }
            if ((s32)texture->offsetS > (s32)(gSpiritDoorLockTexScrollWrap << 8))
            {
                texture->offsetS =
                    (s16)(texture->offsetS - (gSpiritDoorLockTexScrollWrap << 8));
            }
        }
