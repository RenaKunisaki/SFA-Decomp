 ...
 * map-hint state (gArrivedWarpIndex), and per-marker game bits. Markers placed
...
void WarpPoint_update(GameObject* obj) {
    ...
    if (placement->savePointArmed != 0 && state->savePointRecorded == 0 && gArrivedWarpIndex > WARPPOINT_NO_HINT &&
        gArrivedWarpIndex == placement->hintId) {
    ...
    case WARPPOINT_MODE_PROXIMITY:
        if (gArrivedWarpIndex > WARPPOINT_NO_HINT || mainGetBit(GAMEBIT_WarpPointRelatedD53) != 0) {
    ...
    case WARPPOINT_MODE_HINT_TIMER: {
        ...
        f32 deltaZ = player->anim.localPosZ - obj->anim.localPosZ;
        distance = sqrtf(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ);
        if (gArrivedWarpIndex > WARPPOINT_NO_HINT && placement->enabled != 0 && distance < WARPPOINT_HINT_TRIGGER_RADIUS &&
            player->anim.parentAddress == obj->anim.parentAddress) {
    ...
    case WARPPOINT_MODE_GATED_WARP2:
        ...
        if (gArrivedWarpIndex > WARPPOINT_NO_HINT && state->sequenceTriggered == 0 && placement->enabled != 0 &&
            distance < state->triggerRadius && player->anim.parentAddress == obj->anim.parentAddress) {
            (*gObjectTriggerInterface)->runSequence(state->sequenceId, obj, -1);
            gWarpArrivalTimer = WARPPOINT_HINT_TIMER_FRAMES;
            state->sequenceTriggered = 1;
